...
- Provenance:
  ...
  - Follow-up research (2026-07-17, verified + cited): adversarial-vs-role-based review — Refute-or-Promote (arXiv 2604.19049), overcorrection (arXiv 2603.00539), Bun Zig→Rust case, Nine-Judges/correlated-error (arXiv 2605.29800); security-review cadence — NIST SSDF, OWASP SAMM, Meta RADAR (arXiv 2605.30208), Anthropic per-PR security review, curl bug-bounty collapse. Note: two seed citations were found overstated on verification (Refute-or-Promote has no cooperative-review control and disclaims ablation; the "80+ agents" figure was actually ten; the Bun case did include an informal spec-conformance step + a separate post-merge security pass) — the spec relies on the verified residue only.
...
- **WS-3.2 — Collapse the high-risk path 3→2 lenses, constrained by the verified review research.** Merge spec + quality into one `reviewLoop` over the existing two-stage `mergedReviewPrompt` (`mitosis.js:993`), keep the independent `securityReviewPrompt` loop. The merge is only safe under these research-grounded constraints (arXiv 2603.00539 overcorrection; 2604.19049 Refute-or-Promote; 2605.29800 correlated-judge saturation):
  - **Spec-conformance stays a distinct, NON-adversarial comparison** (Stage 1 hard precondition: spec-text vs. diff — list every missing / extra / misunderstood requirement). Adversarial "assume it's wrong" framing measurably DEGRADES conformance judgment (false-rejection rate rising ~36%→88% under demanding-critique prompts), and the deterministic CI gate cannot backstop a missing requirement that has no test — so this lens must never be folded into an adversarial persona and remains human-triaged advisory.
  - **The quality stage may be reframed adversarially** ("assume it fails; find why") — refutation framing genuinely raises defect-finding precision there. Any adversarial finding that is testable routes through the deterministic CI/boundary gate before being treated as confirmed — the validation gate the research shows adversarial consensus REQUIRES (a panel once had 10/10 agents "confirm" a non-existent vulnerability, killed only by one real test).
  - **Do NOT collapse spec+quality into a single adversarial agent**, and **do NOT fold security into the merged prompt** (thins F3). Adding more same-persona adversarial reviewers does not add coverage (shared blind spots; ~9 judges ≈ 2 effective votes); a distinct objective function (spec-comparison vs. bug-hunting vs. security) does.
- **WS-3.5 — Offload generic style/maintainability to static analysis.** Move pure style/lint-shaped concerns out of the LLM lens into the Tier-0 deterministic layer (WS-8.1), spending the LLM-review budget only where judgment is structurally necessary (Optimization-below-Quality).
...
### WS-8 — Security coverage floor (NEW; research-mandated Pillar-1 addition)

Q3 research (NIST SSDF, OWASP SAMM, BSIMM, GitHub Advanced Security, Microsoft SDL, Meta RADAR arXiv 2605.30208, and Anthropic's own per-PR security review) converges on a two-tier standard: an **always-on deterministic floor on 100% of changes** plus a **risk-gated deep review**. Mitosis today has NEITHER a deterministic security floor NOR any security coverage on low-risk tasks, and a code-verified wiring disconnect worsens it. This workstream **ADDS coverage (Quality > Optimization); it is not a cut.** Keep the deep LLM security review SCOPED to the diff with structured findings — the curl bug-bounty collapse (~19/20 AI reports false) is the failure mode of *unscoped* LLM vuln-hunting; do not let a reviewer free-hunt.

- **WS-8.1 — Tier 0: always-on deterministic floor (in CI).** Add to the WS-0 CI gate, on 100% of changes, ~0 LLM tokens: secret scanning + push protection, SAST (Semgrep-class, diff-aware), and dependency/CVE (SCA) scanning (e.g. `osv-scanner` / `npm audit`-equivalent). Lives in `.github/workflows/`, not the per-task loop — no dispatch change, no throughput cost. This is where WS-3.5's offloaded style/lint concerns also land.
- **WS-8.2 — Tier 2: fix the verified review-gate disconnect.** `sensitiveScope(task.fileScope)` (matches `auth`/`security`/`secret`/`payment`/`crypto`/`migrations`/`infra`/`deploy`) is currently wired ONLY into `policyModelFor()` for model-tier selection (`mitosis.js:869-883`), NOT into the review-mode gate (`mitosis.js:1039`). Change the gate so a task escalates to the three-lens (security-inclusive) review when `sensitiveScope(task.fileScope) || irreversible(...) || blastRadius(task) >= K || (any Tier-0 finding)` — reusing already-computed signals. A task touching `auth/` or `payment/` must never receive a no-security review merely because `task.risk === 'low'`. (Verified live gap: `mitosis.js:869-883` vs `:1039`.)
- **WS-8.3 — Tier 1: cheap universal security checklist.** Fold a short OWASP-Top-10-shaped checklist (injection, authz, secrets, SSRF, deserialization, path traversal) into the low-risk `mergedReviewPrompt`, so every task gets a lightweight security pass inside the review call that already runs — no extra dispatch.
- **Model:** the deep security lens stays **Opus** (F3; cheaper models can refuse or under-serve benign security work). Tier 0 is deterministic (no model). Tier 1 rides the existing review model.
- **Acceptance:** Tier-0 scanners fire on a planted secret/SAST/SCA test case in a trivial one-line PR; a low-risk task touching `auth/` provably routes to the security-inclusive review (test on the `:1039` gate); the deep security review stays diff-scoped with structured `file:line`/CWE findings.
...
1. **WS-0** (prereq) — CI + pre-commit wiring **including WS-8.1 Tier-0 security scanners (secret / SAST / SCA)**; commit the gh-scope fix. Everything else depends on the CI backstop.
...
4. **WS-3** + **WS-8.2 / WS-8.3** — review architecture (spec-conformance distinct/non-adversarial, quality adversarial, style → static analysis) plus the security review-gate fix (`:1039`) + Tier-1 checklist (needs `kind:'fix'`; final-review deletion is the pure-deletion prerequisite for WS-5.4).
...
- The security floor exists (WS-8): Tier-0 deterministic scanners (secret / SAST / SCA) run on 100% of changes; the review-mode gate escalates on `sensitiveScope`/`irreversible`/`blastRadius` (not `task.risk` alone); the deep security lens stays Opus and diff-scoped with structured findings.
