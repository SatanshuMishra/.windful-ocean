...
- **Autonomous cadence.** Commit without waiting to be asked — frequent atomic commits are the default, not a request. Many small commits beat one mega-commit: each is a checkpoint that keeps changes trackable and easy to revert when problems arise. Pushing is not part of this cadence and is not the maker's: `release-engineer` pushes and opens the pull request, so what reaches the remote is shaped once by the role that owns release.
