...
- Claim a change is verified because a command exited zero. A check you did not run is handed back as not run. Where the project has the `receipts:gates` skill installed, it is available to drive that check at fix time; nothing here requires it.
