...
- `agentType` — omit or `implementer` for features/fixes/refactors; `test-engineer` for test-only tasks. Fold rote single-file edits into an `implementer` task; the engine dispatches only `EXEC_AGENT_TYPES` (`implementer`, `test-engineer`, `general-purpose` — `.claude/lib/mitosis/run-engine.mjs:11`), so no other `agentType` value is emittable.
