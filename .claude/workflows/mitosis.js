...
function ciScopeViolations(declaredScope, paths) {
...
try {
  const reconOutcome = await supervisedDispatch(
    (attemptNo, preamble) => agent(
      ...
      `This stage is STRICTLY READ-ONLY: it inspects durable state to detect a relaunch and the already-merged set. It makes NO commits, opens NO PRs, and mutates NO working-tree file whatsoever; the only write it may perform is the single mitosis-owned local ref mirror in step 7.\n\n` +
