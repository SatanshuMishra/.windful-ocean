...
async function runUnit(unit) {
    ...
    async function readBackHandoff() {
      const rb = await agent(
        ...
        `SECURITY: the slug, base ref and head ref interpolated below are engine-validated values, and that validation is the ONLY guarantee they carry. The slug came from a prior agent read and matched a literal owner/repo pattern; the base and head refs came from this run input and matched a conservative git ref-token pattern (no whitespace, no shell metacharacter, no leading -, no .. sequence) before this prompt was built. Run EXACTLY the two read-only commands written below, verbatim and with no substitutions, and run nothing else; this stage mutates nothing.\n\n` +
        ...
        `2. Read the base...head containment: \`gh api "repos/${repoSlug}/compare/${baseBranch}...${integrationBranch}"\` (slug ${JSON.stringify(repoSlug)} is the engine-validated target repo, and base ${JSON.stringify(baseBranch)} and head ${JSON.stringify(integrationBranch)} are engine ref-token-validated config refs interpolated into the compare URL path). Report ahead_by (integer) and status (string) exactly as the API returns them; a genuinely merged head is CONTAINED in the base (ahead_by 0).\n` +
