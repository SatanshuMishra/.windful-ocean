  ...
  for (const id of advance.toOpen) {
    ...
    try {
      opened = await agent(
        ...
        `4. Open ONE pull request by running EXACTLY this one command, verbatim, with no substitutions and nothing chained: \`node ${LIB_DIR}/mitosis-git.mjs pr-create --repo ${repoSlug} --head ${integrationBranch} --base ${baseBranch} --title ${JSON.stringify(prTitleFor(id))}\`. That command performs the observe step itself and reuses an existing open PR on this head instead of opening a second, so issue no gh command of your own for this step. Exit 0 prints ONE JSON object carrying action ("reused" or "created") and url; any non-zero exit means nothing was opened. Leave the PR OPEN for a human; perform no merge.\n\n` +
...
async function runUnit(unit) {
    ...
    async function shipOneMsp() {
      ...
      const ship = await agent(
        ...
        `5. Open ONE pull request by running EXACTLY this one command, verbatim, with no substitutions and nothing chained: \`node ${LIB_DIR}/mitosis-git.mjs pr-create --repo ${repoSlug} --head ${integrationBranch} --base ${baseBranch} --title ${JSON.stringify(prTitleFor(msp.id))}${prDependsFlag(msp.dependsOn)}\`. It opens head ${integrationBranch} onto base ${baseBranch}, stacked bottom-up on the already-merged MSPs (${dependsList}). That command performs the observe step itself and reuses an existing open PR on this head instead of opening a second, so issue no gh command of your own for this step. Exit 0 prints ONE JSON object carrying action and url; any non-zero exit means no pull request was opened - report it and stop.\n` +
