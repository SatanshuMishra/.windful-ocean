...
assert_contains "$(jq -r '.hooks.SubagentStart[0].hooks[0].command' "$S")" "subagent-observer.mjs" "SubagentStart wired"
assert_contains "$(jq -r '.hooks.SubagentStop[0].hooks[0].command' "$S")" "subagent-observer.mjs" "SubagentStop wired"
