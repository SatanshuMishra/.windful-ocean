...
- NEVER prefix a Bash command with `cd` into a directory you are already in. The Bash tool's working directory persists across calls, and a `cd` plus a file read in one compound command forces a permission prompt for as long as any `Read()` deny rule exists in settings. Use repo-relative or absolute paths, and state this in dispatch briefs for search-heavy agents.
