...
test('the spawn-site census places the enforcement backstop as a reasoned exception rather than an unrouted site', () => {
  const shim = census().find((entry) => entry.path === 'git/gh-merge-shim.mjs');
...
for (const probe of REFUSALS) {
  test(`pr-tool refuses ${probe.label} before any child starts`, () => {
    const guard = pr.spawnAllowed;
    assert.equal(typeof guard, 'function', 'pr-tool exposes no guarded spawn surface, so its gh spawn consults no policy');
    const marker = scratchMarker('pr-tool');
    refusesExactly(() => guard(probe.binary, shellArgv(marker), { encoding: 'utf8' }, '/bin/sh'), probe.message, marker);
  });
}

test('pr-tool refuses an executable that is not an absolute path, so no PATH lookup decides which file runs', () => {
  const guard = pr.spawnAllowed;
  assert.equal(typeof guard, 'function', 'pr-tool exposes no guarded spawn surface, so its gh spawn consults no policy');
  const marker = scratchMarker('pr-tool-relative');
  refusesExactly(
    () => guard('gh', shellArgv(marker), { encoding: 'utf8' }, 'sh'),
    'pr-tool: refusing to spawn "gh" through "sh"; the resolved executable must be an absolute path so that no PATH lookup decides which file runs',
    marker,
  );
});
