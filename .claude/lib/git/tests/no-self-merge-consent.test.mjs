...
import { PR_TOOL_VERBS, buildGhArgv } from '../pr.mjs';
...
const GIT_LIB_DIR = fileURLToPath(new URL('../', import.meta.url));
const DENY_CLASSIFIER_SHIM = 'gh-merge-shim.mjs';
const SCAN_ANCHOR = 'pr.mjs';

function libTopLevelModuleNames() {
  return readdirSync(GIT_LIB_DIR, { withFileTypes: true })
...
function mergeScanTargets() {
  return readdirSync(GIT_LIB_DIR, { withFileTypes: true })
    ...
    .map((entry) => [entry.name, join(GIT_LIB_DIR, entry.name)]);
}
...
test('the merge scan reads every git lib top-level source, never a test fixture, and never the deny classifier', () => {
  ...
  assert.ok(present.includes(DENY_CLASSIFIER_SHIM), `${GIT_LIB_DIR} carries ${present.length} top-level .mjs file(s) and none of them is named ${DENY_CLASSIFIER_SHIM}, so this directory is not the tree that owns the merge-deny classifier; every coverage claim below would be measured against the wrong surface`);
  const expected = present.filter((name) => name !== DENY_CLASSIFIER_SHIM);
  const scanned = [...names].sort();
  assert.ok(scanned.includes(SCAN_ANCHOR), `the merge scan does not cover ${SCAN_ANCHOR}, the module this file imports buildGhArgv and PR_TOOL_VERBS from; a scan that drops the one module whose argv builder it also enumerates is not reading the lib surface at all`);
  const missing = expected.filter((name) => !scanned.includes(name));
  const unexpected = scanned.filter((name) => !expected.includes(name));
  assert.deepEqual(scanned, expected, `the merge scan covers ${scanned.length} of the ${expected.length} top-level .mjs module(s) that ${GIT_LIB_DIR} actually carries once ${DENY_CLASSIFIER_SHIM} is exempted. Missing from the scan: ${missing.join(', ') || 'none'}. Scanned but absent from the directory: ${unexpected.join(', ') || 'none'}. The expected set is derived here by its own directory read rather than from mergeScanTargets, so a filter regression that quietly shrinks the scanned surface surfaces as a named diff instead of a smaller count that still clears a floor`);
});
...
for (const [label, pattern] of MERGE_INVOCATION_PATTERNS) {
  test(`no git lib top-level source carries ${label} on any surface`, () => {
...
const PR_TOOL_STAGES = Object.freeze(['observe', 'converge', 'read']);
...
test('argvs from enumerated verb/stage pairs are merge-free', () => {
  assert.ok(PR_TOOL_VERBS.length > 0, 'pr.mjs exports an empty PR_TOOL_VERBS table, so this enumeration has no verb to walk; an empty verb table must fail here rather than report a pass earned over zero verbs and zero argvs');
  const stagesBuiltByVerb = new Map(PR_TOOL_VERBS.map((verb) => [verb, []]));
  const built = [];
  for (const verb of PR_TOOL_VERBS) {
    for (const stage of PR_TOOL_STAGES) {
      let argv;
      try {
      ...
      for (const token of argv) {
        assert.equal(MERGE_ARGV_TOKENS.has(token), false, `buildGhArgv(${verb}, ${stage}) put the merge token ${JSON.stringify(token)} into the argv it returns. This enumeration reads ONLY the verb/stage PAIRS it names: the verb axis is the exported PR_TOOL_VERBS, but stage is an internal argument pr.mjs exports nowhere, so PR_TOOL_STAGES here is a local list that nothing keeps in sync with buildGhArgv — an argv the builder composes at a stage this list does not name is never read by this loop, and this assertion makes no claim about it. What enforces merge abstinence at run time is the chokepoint: ghExecTripwire(argv, classifyGhMerge) gates the module's single spawnSync inside spawnAllowed and fails closed, whatever the argv was built from. The source-text ban lives alongside this module's own tests. Merging stays human-gated`);
        for (const tokenPattern of MERGE_ARGV_TOKEN_PATTERNS) {
          assert.equal(tokenPattern.test(token), false, `buildGhArgv(${verb}, ${stage}) put the merge target ${JSON.stringify(token)} into the argv it returns; like the token assertion above, this reads only the enumerated verb/stage pairs and makes no claim about an argv composed at a stage PR_TOOL_STAGES does not name. That the wrapper may read and open pull requests but never merge one is enforced at run time by ghExecTripwire, in front of the single spawnSync inside spawnAllowed`);
        }
      }
    }
  }
  assert.ok(built.length > 0, `the argv scan built no argv at all from ${PR_TOOL_VERBS.length} exported verb(s) across the stages ${PR_TOOL_STAGES.join(', ')}; every pair hit the unknown-pair refusal, so no argv was inspected and a pass here would guard nothing`);
  const unreached = PR_TOOL_VERBS.filter((verb) => stagesBuiltByVerb.get(verb).length === 0);
  assert.deepEqual(unreached, [], `these exported verbs built no argv at any enumerated stage: ${unreached.join(', ')}. stage is an internal argument of buildGhArgv that pr.mjs exports nowhere, so PR_TOOL_STAGES here is a local list that nothing keeps in sync with the builder. A verb that builds nothing therefore means either the builder no longer serves that verb, or the verb is reachable only at a stage this list does not name — and in the second case every argv that verb can compose escapes the merge-token assertions above unread. Name the missing stage in PR_TOOL_STAGES`);
});
