...
export const NON_DISPATCH_LABEL_SOURCES = Object.freeze({
  'coupling-review.mjs': 'its labels name a coupling pair for a report, and it reaches no model: the source carries no dispatch at all, which this census asserts rather than assumes',
  'gh-merge-shim.mjs': 'its labels name the merge argv specimens the exec-allowlist verb probes, and it reaches no model: the source carries no dispatch at all, which this census asserts rather than assumes',
});
...
const NAME_DELIMITERS = Object.freeze(new Set([':', '-']));
...
function resolvePrefix(prefix, table) {
  ...
  for (const name of table.keys()) {
    const matches = prefix === name || (prefix.startsWith(name) && NAME_DELIMITERS.has(prefix[name.length]));
    if (matches && (best === null || name.length > best.length)) best = name;
  }
...
function resolveLabel(entry, index, table) {
  ...
  if (resolved === null) {
    return { error: `transcription-census: ${at} dispatches under the label ${expression}, which no declared transcription, judgment, journal or program name covers; an unclassified dispatch halts rather than falling into a catch-all bucket` };
  }
  return { expression, kinds: resolved.kinds, category: resolved.category, name: resolved.name, parameterized: null };
}

function censusOneSource(entry, table, seen) {
  ...
  if (Object.hasOwn(NON_DISPATCH_LABEL_SOURCES, name)) {
    ...
    seen.inertSources.add(name);
    return { sites: [], helperArguments: [], passThrough: 0, nodeCount: scanned.nodes.length, labelCount: scanned.labels.length, inert: true };
  }
  ...
  for (const node of scanned.nodes) {
    ...
    const declared = LABELLESS_DISPATCHES.find((shape) => shape.signature === signature);
    ...
    seen.labellessShapes.add(signature);
    if (declared.role === 'pass-through') passThrough += 1;
  }
  ...
  for (const index of scanned.labels) {
    ...
    if (resolved.parameterized !== null) seen.parameterized.add(resolved.parameterized);
    ...
    if (placed.role === 'helper-argument') {
      seen.helpers.add(placed.helper);
  ...
  return { sites, helperArguments, passThrough, nodeCount: scanned.nodes.length, labelCount: scanned.labels.length, inert: false };
}
...
export function censusTranscriptionSources(sources, declared = TRANSCRIPTION_KINDS) {
  ...
  const seen = {
    parameterized: new Set(),
    labellessShapes: new Set(),
    inertSources: new Set(),
    helpers: new Set(),
  };
  const sites = [];
  const helperArguments = [];
  let dispatchNodeCount = 0;
  let labelTokenCount = 0;
  let passThroughCount = 0;
  for (const entry of sources) {
    const measured = censusOneSource(entry, table, seen);
    if (measured.error !== undefined) return halt(measured.error);
    sites.push(...measured.sites);
    helperArguments.push(...measured.helperArguments);
    dispatchNodeCount += measured.nodeCount;
    labelTokenCount += measured.labelCount;
    passThroughCount += measured.passThrough;
  }
