...
async function runWithRealPorts(spec) {
  const io = stubIo(spec);
  const code = await runCli(fullArgv(), io, (config) => realPorts(config, inertDeps()));
  return { code, stderr: io.errOut.join(''), stdout: io.out.join('') };
}

test('UNDISPATCHABLE UNIT: a spec unit carrying no request reports its cause on stderr instead of parking silently', async () => {
  const result = await runWithRealPorts(specWithUnit({}));
...
test('UNDISPATCHABLE UNIT: a request that is not an object reports its cause on stderr', async () => {
  for (const request of ['a string', 42, null, ['an', 'array']]) {
    const result = await runWithRealPorts(specWithUnit({ request }));
...
test('HUMAN ESCALATION STAYS QUIET: a unit that parks for a human writes nothing to stderr', async () => {
  const io = stubIo(specWithUnit({ request: { prompt: 'do alpha' } }));
  const code = await runCli(fullArgv(), io, () => parkingPorts());
  assert.equal(io.errOut.join(''), '');
  assert.match(io.out.join(''), /"state": "parked"/);
  assert.equal(code, EXIT_INCOMPLETE);
});

test('MALFORMED REQUEST FIELDS: every dispatch validation failure names the unit and the field on stderr', async () => {
  const malformed = [
    ['prompt', { prompt: 42 }],
