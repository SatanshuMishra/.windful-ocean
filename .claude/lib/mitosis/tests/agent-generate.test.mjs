...
test('shipped store: --check is clean over the shipped store and the bodies it ships', () => {
  const result = run(['--check', '--store', SHIPPED_STORE, '--agents', SHIPPED_AGENTS]);
  assert.equal(result.code, 0, result.output);
  assert.doesNotMatch(result.output, /zero agent specs/, 'the shipped store reported no specs, so this check compared nothing and proves nothing');
