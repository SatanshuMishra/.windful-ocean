...
export function composeDecomposePrompt(input) {
  ...
  return `You are the decomposition stage of a mitosis run. You have NO Skill tool; follow these instructions directly.\n\n` +
    ...
    `Order the MSPs BOTTOM-UP: an MSP must appear AFTER every MSP it depends on. dependsOn is a SEMANTIC dependency: MSP B depends on MSP A only when B's code cannot compile or run without A's code already existing — B imports a symbol A defines, extends a type A introduces, or calls an API A must ship first. Two MSPs that merely edit the same file are NOT a dependency: that overlap is expressed through fileScope alone, which already serializes them, so declaring a dependency to protect against a same-file collision is unnecessary and harmful — a real dependency also parks every MSP that transitively depends on it should the depended-on MSP fail or stall, dragging an unrelated MSP into a blast radius it never needed. Assign each MSP a stable kebab-case id of 30 characters or fewer, unique within this run.\n\n` +
    ...
    `rationale is one sentence of 200 characters or fewer, printable ASCII only, starting with a capital letter and ending in a full stop — it becomes the Why line of this MSP's pull-request body.\n` +
    ...
    `securityReviewRequired is a boolean you MUST declare on every MSP; it decides whether a dedicated security lens is dispatched over that MSP's diff, and the engine refuses an MSP that leaves the question open rather than settling it by a default nobody wrote. Declare it true when the MSP touches authentication or authorization, input handling, data access, secrets, or external integrations; declare it false otherwise. Judge the MSP's own change, not the subsystem it sits in, and when the change genuinely straddles one of those surfaces declare true — a security lens on a benign diff costs one dispatch, while a skipped one on a real trust boundary is a review the run reports as having happened and never ran.\n\n` +
    ...
    `Return ONLY the structured object: { msps: [ { id, title, rationale, changeType, scope, securityReviewRequired, dependsOn, fileScope: { edit, read, truncated } } ] }, ordered bottom-up.`;
}
