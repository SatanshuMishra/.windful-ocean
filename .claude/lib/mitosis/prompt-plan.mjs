...
export function composePlanPrompt(input) {
  ...
  return `You are the planning stage for MSP "${unitId}" (${title}) of a mitosis run. You have NO Skill tool.\n\n` +
    `Locate the superpowers writing-plans skill WITHOUT hardcoding its version: run \`node ${shellQuote(`${libDir}/superpowers-prompts.mjs`)}\` if it prints a skillsDir, otherwise glob \`${shellQuote(writingPlansGlob)}\`. Read that SKILL.md and follow it exactly.\n\n` +
